...
**quantumready** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `quantumready` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
quantumready ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
quantumready ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
quantumready ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `quantumready` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="quantumready") for rec in records]   # records = your JSON output
