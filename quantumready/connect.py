"""Native cognis-connect emit for quantumready — forward findings to any platform.

Maps quantumready's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    quantumready ... --format json | quantumready-emit --to stix
    quantumready-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('id', '') + ' - ' + rec.get('label', '')
